...
vec4 calcHexInfo(vec2 uv)
{
    ...
    vec4 offset = vec4(uv - hexCenter.xy * hexSide, uv - (hexCenter.zw + 0.5) * s);
...
void main(void)
{
    ...
    mediump float dist = sdHexagon(hexGrid.yx * g_GridSize, radius); // xy axis swapped to rotate the hexagons by 30 degrees (so they neatly fit into each other)
